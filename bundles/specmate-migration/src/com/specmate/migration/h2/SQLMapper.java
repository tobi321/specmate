...
public abstract class SQLMapper {
	...
	protected List<String> insertExternalObjectReference(String objectName) throws SpecmateException {
		int id = SQLUtil.getIntResult("SELECT id FROM CDO_EXTERNAL_REFS ORDER BY id ASC", 1, connection);
	...
	protected List<String> insertExternalAttributeReferences(String objectName, List<String> attributeNames)
		...
		// queries.add(getInsertExternalReferenceQuery(baseUri, id)); --> BUG:
		// when adding a new attribute, no need to add another object reference
